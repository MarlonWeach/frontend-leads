...
async function fetchRealOpenAIUsage(days: number): Promise<BillingResponse | null> {
  ...
  for (const bucket of usageData) {
    ...
    dayTokens = dayInputTokens + dayOutputTokens;

    // Encontrar custo correspondente
    const costBucket = costsData.find((c: any) => c.start_time === bucket.start_time);
    const dayCost = costBucket?.results?.[0]?.amount?.value || 0;
  ...
  // Definir limites (valores padrão - usuário deve configurar)
  const softLimit = 100; // $100 USD
  const hardLimit = 200; // $200 USD
  const usagePercentage = (totalCost / softLimit) * 100;
  ...
  if (usagePercentage > 80) {
    alerts.push({
      type: 'warning' as const,
      message: `Uso atual (${usagePercentage.toFixed(1)}%) se aproxima do limite configurado`,
      threshold: 80
    });
  }
  if (usagePercentage > 95) {
