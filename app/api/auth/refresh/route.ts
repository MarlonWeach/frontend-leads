...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!refreshToken) {
      logAuthAudit({ event: 'session_refresh_failure', request, status: 401, reason: 'missing_refresh_token' });
    ...
    logAuthAudit({
      ...
      request,
      status: 500,
      reason: error instanceof Error ? error.message : 'unexpected_refresh_error',
    });
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Erro inesperado de refresh.' },
      { status: 500 }
    );
  }
}
